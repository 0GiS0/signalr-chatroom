document.addEventListener('DOMContentLoaded', function() {
	...
	function createMessageEntry(encodedName, encodedMsg, currentTime, ugly, private) {
		...
		if (encodedName === '_SYSTEM_') {
			...
			entry.innerHTML =
				`<div class="message-avatar pull-right">${encodedName}<br/><i style="font-size:x-small; float:right;">${new Date().toLocaleTimeString()}</i></div>` +
			...
			entry.innerHTML =
				`<div class="message-avatar pull-left">${encodedName}</div>` +
			...
			if (ugly) {
				terms.forEach((term) => {
					...
					encodedMsg = encodedMsg.replace(
						regEx,
						`${term.substring(0, 1)}${term.substring(1).replace(/./g, '*')}`
					);
				});
			}
